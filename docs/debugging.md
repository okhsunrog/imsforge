...
`su -M NUMERIC_UID -c 'id; ...'`. UID должен идти перед `-c`: в этой реализации
аргументы после команды могут стать частью команды shell. Это полезный контроль, но **не точная копия
