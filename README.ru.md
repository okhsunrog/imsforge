...
загрузке. Поэтому WebUI прогоняет введённое через `imsforge check` до того, как заменить рабочий
файл; руками это делается так же: `imsforge check --config /data/adb/imsforge/carriers.json`.
...
Каждый прогон дополнительно оставляет `/data/adb/imsforge/status.json`: что именно он решил по
каждому оператору и почему. Это и читает WebUI — лог он не разбирает, так что формулировка строки
лога не является интерфейсом. `imsforge status` печатает эту запись вместе с тем, читает ли
система прямо сейчас именно наш файл — то же сравнение отпечатков, что и выше.
...
- Проверено на Pixel 8 Pro (husky), Android 17, KernelSU Next с NoMount. Magisk по
